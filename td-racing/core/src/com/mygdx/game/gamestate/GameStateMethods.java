...
public class GameStateMethods {
	 ...
	 * @param keyCode (Keys.F11 for example)
	 */
	 ...
	 */
	public static Vector2 calculateCenteredTextPositon(final BitmapFont font, final String text, final float width, final float height) {
	 ...
	 * Get the coordinates to render text centered
	 * @param font - BitmapFont which will be used to draw the text
	 * @param text - texts that should be displayed
	 * @param width - Width of screen
	 * @param height - Height of screen
	 * @return x and y coordinates for drawing the texts
	 */
	public static Vector2[] calculateCenteredMultiLineTextPositons(final BitmapFont font, final String[] text, final float width, final float height) {
		final Vector2[] positions = new Vector2[text.length];
		for (int i = 0; i < text.length; i++) {
			final GlyphLayout temp = new GlyphLayout(font, text[i]);
			positions[i] = new Vector2(width / 2 - temp.width / 2, height / (text.length + 1) * (text.length - i) + temp.height / 2);
		}
		return positions;
	}
